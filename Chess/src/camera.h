...
public:
    ...
    glm::mat4 GetProjectionMatrix(float fov=45.0, float ratio=1.0, float near=0.01, float far=100.0) { return glm::perspective(fov, ratio, near, far); }
    ...
    void ProcessKeyboardRotation(float YawRot, float PitchRot, float deltaTime, GLboolean constrainPitch = true) {
        float velocity = MovementSpeed * deltaTime;
        Yaw += YawRot * velocity; Pitch += PitchRot * velocity;
        if (constrainPitch) { if (Pitch > 89.0f) Pitch = 89.0f; if (Pitch < -89.0f) Pitch = -89.0f; }
        updateCameraVectors();
    }
